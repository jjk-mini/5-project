...
const NewBookingPage = () => {
  ...
  const fetchRooms = async () => {
    try {
      ...
      if (availableRooms.length === 0) {
        toast("No available rooms found", { icon: "ℹ️" });
      }
    } catch (error) {
  ...
  // Since we don't have a get all users endpoint, we'll use a workaround
  const fetchExistingGuests = async () => {
    try {
      // Try to get the current user profile
      const response = await authApi.getCurrentUser();
      if (response && response.user) {
        // If the current user is a guest, add them to the list
        if (response.user.role === "guest") {
          setExistingGuests([response.user]);
        }
      }
      console.log("Guest list loaded (if any). Guests can be added manually.");
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Send guest details directly with the booking
      const bookingData = {
        room: formData.room,
        ...
        // Send guest details as an object
        guest: {
      ...
      const errorMessage = error.response?.data?.message || "Failed to create booking. Please check all fields.";
  ...
  const filteredGuests = existingGuests.filter((g) =>
    g.name?.toLowerCase().includes(searchGuestTerm.toLowerCase()) ||
    g.email?.toLowerCase().includes(searchGuestTerm.toLowerCase())
  );
        ...
        {success ? (
          ...
          >
            ...
            <p style={{ color: COLORS.TEXT_SECONDARY, marginTop: "8px" }}>
              The booking has been created and confirmed.
            </p>
            <button
          ...
          >
            <form onSubmit={handleSubmit}>
                      ...
                      {fetching ? (
                        ...
                        rooms.map((room) => (
                          <option key={room._id} value={room._id}>
                            {room.roomNumber} - {room.type} (PKR {room.price?.toLocaleString()}/night)
                          </option>
                        ))
                      )}
                    </select>
              ...
              >
                ...
                >
                  <Save size={18} />
                  {loading || creatingGuest ? "Creating..." : "Create Booking"}
                </button>
              </div>
            </form>
          </motion.div>
